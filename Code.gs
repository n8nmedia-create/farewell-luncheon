...
function setupEventDayReminder() {
  ...
  ScriptApp.getProjectTriggers().forEach(t=>{
    if(t.getHandlerFunction()==='sendEventDayReminders') ScriptApp.deleteTrigger(t);
  });
  // Schedule for March 31, 2025 at 10:00 AM Korea time (UTC+9 = 01:00 UTC)
  ScriptApp.newTrigger('sendEventDayReminders')
    .timeBased()
    .atDate(2025,3,31)
    .atHour(1)       // 1 AM UTC = 10 AM KST
    .create();
  Logger.log('✅ Event-day reminder scheduled for March 31 at 10:00 AM KST');
}
...
function sendEventDayReminders() {
  ...
  let sent=0;
  orders.forEach(o=>{
    try { sendReminderEmail(o); sent++; } catch(e){ Logger.log('Day-of reminder err: '+e); }
  });
  Logger.log('📩 Sent '+sent+' event-day reminders.');
}

function sendReminderEmail(o) {
