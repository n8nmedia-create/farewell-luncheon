...
const SHEET_ID        = '11GDotTC3Pu8oZdTcvLg0Oe4sKPhnK69tZvrrT7N3G9s';   // ← your Sheet ID
...
<div style="max-width:560px;margin:32px auto;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 20px rgba(0,0,0,.09)">
  ...
  <div style="padding:28px">
    ...
    <div style="text-align:center;margin-bottom:10px">
      <a href="https://www.google.com/calendar/render?action=TEMPLATE&text=Farewell+Luncheon+for+${encodeURIComponent(EVENT.honoree)}&dates=20250331T020000Z/20250331T060000Z&location=${encodeURIComponent(EVENT.location)}&details=${encodeURIComponent(EVENT.farewell)}" style="background:#1c1917;color:#faf7f2;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:13px;font-weight:500;display:inline-block">📅 Add to Calendar</a>
    </div>
...
function addOrganizerManual() {
  const EMAIL_TO_ADD='organizer@anyemail.com'; // ← CHANGE then Run
  const ss=SpreadsheetApp.openById(SHEET_ID);
  ...
  if(sheet.getLastRow()>1&&sheet.getRange(2,1,sheet.getLastRow()-1,1).getValues().flat().includes(EMAIL_TO_ADD)){Logger.log('⚠️ Already exists: '+EMAIL_TO_ADD);return;}
  const token=generateToken();
  sheet.appendRow([EMAIL_TO_ADD,token,'Active',new Date().toLocaleDateString()]);
  sheet.autoResizeColumns(1,4);
  Logger.log('✅ Access granted: '+EMAIL_TO_ADD);
  Logger.log('📋 Dashboard link:\n'+DASHBOARD_URL+'?token='+token);
}

function revokeOrganizerManual() {
  const EMAIL='organizer@anyemail.com'; // ← CHANGE then Run
  const ss=SpreadsheetApp.openById(SHEET_ID);
  const sheet=ss.getSheetByName(ORGANIZERS_TAB);
